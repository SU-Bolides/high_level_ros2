...
class WallFollowPID(Node):
    def __init__(self):
        ...
        self.declare_parameter('max_steering_angle', 60.0)  # Angle de braquage max (degrés)
    ...
    def emergency_callback(self, msg):
        ...
        if self.emergency_stop:
            self.get_logger().warn("EMERGENCY STOP activated - Wall following disabled")
        else:
            self.get_logger().info("Emergency stop cleared - Wall following resumed")

    def scan_callback(self, msg):
        if self.emergency_stop:
            # Arrêt d'urgence : vitesse et direction à zéro
            direction_cmd = 0.0
            speed_cmd = 0.0
            wall_distance = 0.0
            error = 0.0
            self.get_logger().warn("Emergency stop active - vehicle stopped")
        else:
        ...
        # Vérifier obstacles devant (autour de 180° : -15° à +15°, soit 165° à 195°)
            ...
            if front_distances:
                ...
                # Obstacle devant : ralentir et tourner plus fort
                    ...
                    direction_cmd *= 5.0  # Augmenter la correction
                    direction_cmd = max(-self.max_steering_angle, min(self.max_steering_angle, direction_cmd))
